...
def prepare_batch(batch, in_sequencer, bio_sequencer, rel_sequencer, relation_to_idx):
    ...
    rel_tensor = rel_sequencer.create_padded_tensor(rel_tags, relation_to_idx)  # make a part of sequencer
...
def val_loop(model, loss_fn_bio, loss_fn_rel, dataloader, bio_sequencer, device):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            # Permute bio to (batch size, labels, seq len)  #  put in model
    ...
    # get padding working with model (it's currently predicting padding)
